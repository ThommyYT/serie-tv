...
function Test-ChocolateyInstalled {
    ...
    # Restituiamo sempre false per forzare l'installazione locale
    # ignorando versioni di sistema o cartelle esistenti.
    return $false
}
...
# 1. NON resettare il PATH dal sistema, mantieni quello corrente del processo
if ($env:Path -notlike "*$chocoExePath*") {
    $env:Path = "$env:Path;$chocoExePath"
}

Write-Host 'Ensuring chocolatey.nupkg is in the lib folder'
# 2. Forza il percorso verso la tua cartella locale TOOLS
...
# 3. Rimuovi o commenta la riga Copy-Item se continua a darti problemi, 
# oppure assicurati che $file sia definito correttamente.

...
export const initArchiveLogic = async (response: ApiResponseSite) => {
    ...
    await siteLogicHTML(main, response, 'archive');

    initSiteLogic((page: Number) => {
        main.empty().append(loadingElement);
        main.show();
        $.post('./php/siteArchive.php', { p: page }, (response: ApiResponseSite) => {
            main.fadeOut(600, () => {
                main.empty();
                // Chiamiamo la logica di inizializzazione/rendering passando i dati
                initArchiveLogic(response);
            ...
            console.error("Errore nel caricamento dati da: ./php/siteArchive.php");
        });
    });
};
